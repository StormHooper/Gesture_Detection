...
last_printed = None
frame_counter = 0
angle_print_interval = 30  # Print every ____ frames
...
def calculate_hand_yaw(landmarks):
    wrist = landmarks[0]
    index_base = landmarks[5]
    pinky_base = landmarks[17]
    vx = index_base.x - pinky_base.x
    vy = index_base.y - pinky_base.y
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        for lm_set in results.multi_hand_landmarks:
            ...
            # --- Print angle every N frames ---
            if frame_counter % angle_print_interval == 0:
                yaw_angle = calculate_hand_yaw(lm)
